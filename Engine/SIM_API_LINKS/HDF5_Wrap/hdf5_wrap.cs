...
namespace HDF5
{
    ...
    {//
        ...
        //todo : parameterize to provide greater control...
            ...
            public double[,] hdfGetDataSeries(string sGroupName, string sDataSetName, bool bIsReadOnly = true)
            {
                ...
                if (this._bHDF_IsOpen)                                                      //HDF file must be open when passed.   
                {
                    ...
                    if (hdfIsValidGroup(sGroupName, false))              //get to group
                    {
                        ...
                        {
                            Console.Write("Invalid Dataset");
                            Console.ReadLine();
                        }
                    }
                    else
                    {
                        Console.Write("Invalid Group");
                        Console.ReadLine();
                    }

                    //H5F.close(_h5loc);        //TODO need to close the file, but i thought we could 
                }
         ...
         public bool hdfCreateHDF(string sHDF_FilePath)
            ...
            public bool hdfWriteDataSeries(double[,] dArrOut, string sGroupName, string sDataSetNameOut)
            {
                ...
                if (hdfCanWriteHDF())
                {
                    ...
                    if (hdfIsValidGroup(sGroupName, true))
                    {
                        _h5g = H5G.open(_h5loc, sGroupName);
                        ...
                        Console.Write("Write Successful");
                        Console.ReadLine();
                    }
